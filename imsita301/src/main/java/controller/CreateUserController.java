...
public class CreateUserController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!userService.isEmailValid(email)) {
            request.setAttribute("mess", "Mail must be end with @fpt.edu.vn");
            request.getRequestDispatcher("registerAccountEmail.jsp").forward(request, response);
        }
        ...
        if (!udb.checkEmail(email) && userService.isEmailValid(email) && userService.isPasswordValid(password, confirmPassword)) {
            utils.SendMail mail = new SendMail();
            String cap = mail.createCaptcha();
            mail.send(email, "Create new profile", "Here is your OTP: " + cap + "\nPlease use this to complete your registration.");
